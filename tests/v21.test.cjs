...
test('app fetches sufficient native history for higher frames and only derives M3',()=>{const src=require('fs').readFileSync(require('path').join(__dirname,'../app/src/main/assets/app.js'),'utf8');const decl=src.split('\n').find(line=>line.includes('intervals={'));assert.ok(decl);for(const token of ["M1:'1m'","M5:'5m'","M15:'15m'","H1:'1h'","H4:'4h'"])assert.ok(decl.includes(token));assert.ok(src.includes("st.bars.M3=D.aggregate(st.bars.M1||[],180,60)"));assert.equal(src.includes("D.aggregate(st.bars.M1||[],3600,60)"),false);assert.equal(src.includes("D.aggregate(st.bars.M1||[],14400,60)"),false);});
