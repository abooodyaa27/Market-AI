...
test('app derives M15 from M5 rather than requesting provider M15 bars',()=>{const src=require('fs').readFileSync(require('path').join(__dirname,'../app/src/main/assets/app.js'),'utf8');const m=src.match(/const intervals=\{([^}]+)\}/);assert.ok(m);assert.doesNotMatch(m[1],/M15\s*:\s*'15m'/);assert.match(src,/D\.aggregate\(st\.bars\.M5\|\|\[\],900,300\)/);});
