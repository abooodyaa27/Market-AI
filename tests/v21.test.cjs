...
test('AI cold start is reduced but still requires temporal validation and test',()=>{assert.equal(T.MIN_ROWS,60);assert.ok(T.MIN_TRAIN>=30);assert.ok(T.MIN_VALIDATION>=10);assert.ok(T.MIN_TEST>=10);assert.ok(T.MIN_TRADES>=5);});
