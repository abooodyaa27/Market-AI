...
test('app derives M15 from M5 rather than requesting provider M15 bars',()=>{const src=require('fs').readFileSync(require('path').join(__dirname,'../app/src/main/assets/app.js'),'utf8');assert.match(src,/const intervals=\{M1:'1m',M5:'5m',H1:'1h',H4:'4h'\}/);assert.match(src,/st\.bars\.M15=D\.aggregate\(st\.bars\.M5\|\|\[\],900,300\)/);});
