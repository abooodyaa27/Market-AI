...
test('XAUUSD M15 accepts a real session break but rejects missing intraday candles',()=>{assert.equal(F.gapKind('XAUUSD','M15',8*3600,900),'SESSION');assert.equal(F.gapKind('XAUUSD','M15',1800,900),'SESSION');assert.equal(F.gapKind('BTCUSD','M15',8*3600,900),'BAD');});
