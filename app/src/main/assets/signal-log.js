...
'use strict';
const KEY='market_ai_v21_signals',MAX=500,TTL=4*60*60*1000;
...
function add(symbol,a,now=Date.now()){
 ...
 const row={id:makeId(symbol,a,now),symbol,openedAt:now,closedAt:null,side:a.decision,type:a.opportunity,score:a.score,entry:a.trade.entry,sl:a.trade.sl,tp1:a.trade.tp1,tp2:a.trade.tp2,tp3:a.trade.tp3,atr:a.meta?.atr||null,poi:a.poi||null,stages:a.stages,status:'OPEN',highestTarget:null,maxFavorable:0,maxAdverse:0,r:0};rows.push(row);save(rows);return row;
}
function update(symbol,price,now=Date.now()){
 ...
 if(x.highestTarget==='TP3'){x.status='TP3';x.closedAt=now;x.r=risk?2.5:0;}
 else if((price-x.sl)*d<=0){x.status=x.highestTarget||'SL';x.closedAt=now;x.r=x.highestTarget==='TP2'?1.75:x.highestTarget==='TP1'?1:risk?-1:0;}
 else if(now-x.openedAt>=TTL){x.status=x.highestTarget||'EXPIRED';x.closedAt=now;x.r=x.highestTarget==='TP2'?1.75:x.highestTarget==='TP1'?1:(risk?move/risk:0);}
 changed=true;
 }
...
function csv(){const rows=load(),h=['id','symbol','openedAt','closedAt','side','type','score','entry','sl','tp1','tp2','tp3','status','highestTarget','maxFavorable','maxAdverse','r'];return [h.join(','),...rows.map(x=>h.map(k=>JSON.stringify(x[k]??'')).join(','))].join('\n');}
