/* Local signal journal for Market AI Scalp AI V1.2. */
...
'use strict';
const KEY='market_ai_scalp_v12_signals',MAX=500,TTL=4*60*60*1000;
...
function csv(){const rows=load(),h=['id','symbol','openedAt','closedAt','side','type','score','aiReview','aiProbability','entry','sl','tp1','tp2','tp3','status','highestTarget','maxFavorable','maxAdverse','r'];return [h.join(','),...rows.map(x=>h.map(k=>JSON.stringify(x[k]??'')).join(','))].join('\n');}
