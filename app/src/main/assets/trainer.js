...
function progress(rows,symbol,evaluatedUntil=0){const data=collect(rows,symbol),opportunities=groups(data.rows);return{samples:data.rows.length,opportunities:opportunities.length,newHoldout:evaluatedUntil>0?opportunities.filter(g=>g.at>evaluatedUntil).length:null,requiredNewHoldout:10,excluded:data.excluded};}
