'use strict';
(() => {
const A=window.MarketAnalysis,D=window.MarketData,$=id=>document.getElementById(id);
...
async function refreshBars(s){const st=store[s];if(st.barBusy)return;st.barBusy=true;st.lastBars=Date.now();try{await Promise.all(frames.map(async name=>{try{const values=D.normalizeBars(await request(s+'/ohlc?interval='+intervals[name]+'&limit=120'),name);if(values.length<55)throw Error('شموع غير كافية');st.bars[name]=values;st.barTimes[name]=Date.now();delete st.errors[name];
 ...
 }catch(e){st.errors[name]=e.name==='AbortError'?'انتهت مهلة الاتصال':e.message;}}));}finally{st.barBusy=false;if(asset===s)render();}}
function render(){const st=store[asset],now=Date.now(),t=st.tick,live=freshTick(t,now)&&!st.errors.tick;
 ...
 const an=A.analyze({symbol:asset,bars:ready?st.bars:{},tick:live?t:{},now});
 ...
 chip('bias','H4: '+an.bias,an.bias==='BULLISH'?'good':an.bias==='BEARISH'?'bad':'warn');
 ...
 $('reason').textContent=an.why;$('waiting').textContent=an.decision==='WAIT'?an.why:'إشارة تحليلية فقط. مستويات إرشادية قد تتغير مع السعر؛ لا توجد صفقة منفذة.';
...
function poll(){const now=Date.now();for(const s of assets){patch(s,now);refreshTick(s);const st=store[s],m1=st.bars.M1||[],lastClosed=m1.filter(b=>!b[5]).slice(-1)[0];const newBar=!lastClosed||lastClosed[0]<Math.floor(now/60000)*60-60;
