...
const assets=['XAUUSD','BTCUSD'],frames=A.FRAMES,intervals={M15:'15m',M5:'5m',M1:'1m'};
...
async function refreshTick(s){const st=store[s];if(st.tickBusy)return;st.tickBusy=true;try{const value=D.normalizeTick(await request(s),Date.now());if(!value)throw Error('سعر غير صالح');st.tick=value;delete st.errors.tick;patch(s,Date.now());J.update(s,value.price,Date.now());AI.updateShadow(s,value.price,Date.now());AI.learn(AI.allTrainingRows(J.load()));}catch(e){st.errors.tick=e.name==='AbortError'?'انتهت مهلة الاتصال':e.message;}finally{st.tickBusy=false;if(asset===s)render();}}
...
function renderHistory(){const filter=x=>x.symbol===asset,st=J.stats(filter),rows=J.load().filter(filter).slice(-10).reverse();$('historyScope').textContent=asset+' • محفوظ محليًا';$('stTotal').textContent=st.total;$('stWin').textContent=st.winRate+'%';$('stR').textContent=st.avgR.toFixed(2);$('stTp1').textContent=st.tp1+'%';$('stTp2').textContent=st.tp2+'%';$('stTp3').textContent=st.tp3+'%';$('stSl').textContent=st.sl+'%';$('history').innerHTML=rows.length?rows.map(x=>'<div class="row"><strong>'+x.symbol+' '+x.side+'</strong> • '+x.type+' • '+x.score+'/100<br><span class="muted">سبب الدخول: '+((x.entryBasis||[]).join(' • ')||'—')+'</span><br>'+new Date(x.openedAt).toLocaleString('ar-SA')+' • '+x.status+'</div>').join(''):'<div class="row muted">لا توجد إشارات مسجلة لهذا الأصل بعد.</div>';}
...
function render(){const st=store[asset],now=Date.now(),t=st.tick,live=freshTick(t,now)&&!st.errors.tick,ready=frames.every(n=>st.barTimes[n]&&now-st.barTimes[n]<90000&&!st.errors[n]);const raw=A.analyze({symbol:asset,bars:ready?st.live:{},tick:live?t:{},now});const wrapped=applyAI(asset,raw),an=wrapped.analysis,review=wrapped.review;maybeLog(asset,an);
