...
async function refreshBars(s){const st=store[s];if(st.barBusy)return;st.barBusy=true;st.lastBars=Date.now();try{await Promise.all(chartFrames.filter(name=>name!=='M3').map(async name=>{try{const values=D.normalizeBars(await request(s+'/ohlc?interval='+intervals[name]+'&limit=120'),name);if(values.length<(analysisFrames.includes(name)?55:20))throw Error('شموع غير كافية');st.bars[name]=values;st.barTimes[name]=Date.now();delete st.errors[name];const old=st.live[name]?.slice(-1)[0],current=values.at(-1),merged=values.map(b=>b.slice());if(old&&current&&old[0]===current[0]&&current[5]){const x=merged.at(-1);x[2]=Math.max(x[2],old[2]);x[3]=Math.min(x[3],old[3]);}st.live[name]=merged;patch(s,Date.now());}catch(e){st.errors[name]=e.name==='AbortError'?'انتهت مهلة الاتصال':e.message;}}));if(st.bars.M1?.length){st.bars.M3=D.aggregate(st.bars.M1,180);st.live.M3=D.aggregate(st.live.M1||st.bars.M1,180);st.barTimes.M3=st.barTimes.M1;delete st.errors.M3;}}finally{st.barBusy=false;if(asset===s)render();}}
...
$('toolEma').addEventListener('click',()=>{const cur=window.MarketChart.getEMA().join(', '),v=prompt('أدخل قيم EMA مفصولة بفواصل — مثال: 9, 21, 50, 200',cur);if(v!==null){window.MarketChart.setEMA(v.split(/[,، ]+/));chip('toolEma','EMA '+window.MarketChart.getEMA().join('/'),'good');}});
